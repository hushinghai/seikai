...
/* Conversion Config */
s.currencyCode="Rupees"
/* Link Tracking Config */
s.trackDownloadLinks=true
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx"
s.linkInternalFilters="javascript:,.eventbrite.com"
...
s.searchTermVariable		        = 'eVar5';

function s_doPlugins(s) {
/* Add calls to plugins here */
s.eVar14 = s.prop26 = window.location.href;
s.eVar16 = s.prop27 = document.referrer;
	if(!s.campaign) {
		s.campaign=s.getQueryParam('ref');
		var ref_code = s.campaign;
	}
	if(!s.eVar2) {
	...
	if(s.prop5) {
	    ...
	    source.push(s.getQueryParam('utm_source'));
	    source.push(s.getQueryParam('utm_campaign'));
	    source.push(s.getQueryParam('utm_term'));
	    source = source.join(': ');
	    if(source.replace(/\:/g, '').replace(/ /g, '') != '') {
		s.eVar2 = s.prop28 = source;
	    }
	}

/*AUTOMATE PRODUCT FINDING METHODS*/
	if(s.campaign&&!s.eVar2){
		s.eVar2="external campaign";
	}
	if(s.eVar5&&!s.eVar2){
		s.eVar2="internal search";
	}
	if(s.eVar1&&!s.eVar2){
		s.eVar2="internal campaign";
	}
	if((s.pageName=="wait list")&&!s.eVar2){
		s.eVar2="wait list";
	}
	if((s.pageName=="recommender")&&!s.eVar2){
		s.eVar2="recommender";
	}
	if((s.pageName=="directory")&&!s.eVar2){
		s.eVar2="directory";
	}


/*OTHER EXTERNAL SITES*/
	if(document.referrer&&!s.eVar2)
	{
...
/*Get Original Search Location*/
	 var intSearch=s.getPreviousValue(s.pageName,'gpv_p8','');
	 if(s.prop5&&(s.events=="event1")){
		s.prop8=intSearch;
	 }
...
s.visitorNamespace="eventbrite"
s.dc="122"
/*s.trackingServer="metrics.eventbrite.com"
s.trackingServerSecure="smetrics.eventbrite.com"*/

/* Helper functions */
function linkPrintTicket(obj, orderID) {
    var s=s_gi(s_account);
...
function linkCreateEvent(obj, source) {
    var s = s_gi(s_account);
    s.linkTrackVars = 'prop14';
    s.prop14 = source;
    s.tl(obj, 'o', 'createEvent');
}

function linkEventStatus(obj, eventID, status) {
    var s = s_gi(s_account);
    s.linkTrackVars = 'products,events,eVar10';
    s.linkTrackEvents = 'event19';
    s.products = ';' + eventID;
    s.events = 'event19:' + eventID;
    s.eVar10 = status;
    s.tl(obj,'o','eventStatus');
}
